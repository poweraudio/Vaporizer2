...
void VASTQFilter::filterTypeChanged(OwnedArray<VASTSynthesiserVoice>* voices, int filter, int ftype, int fsubtype, int ftype2, int fsubtype2, int ftype3, int fsubtype3, int osFactor, bool isUI, CVASTVcf* uiVCF, CVASTSettings* m_Set) {
	if (!isUI) {
		for (auto* voice : *voices) {
			//check prepareforplay comb filter!!

			CVASTSingleNote* note = dynamic_cast<CVASTSingleNote*>(voice);
			if (note == nullptr) {
				vassertfalse; //error state - thread problem
				continue;
			}

			//Filter A
			if (ftype != fut_none) {
				memset(&note->m_VCF[filter]->FBP.FU[0], 0, sizeof(note->m_VCF[filter]->FBP.FU[0]));
				memset(&note->m_VCF[filter]->FBP.FU[1], 0, sizeof(note->m_VCF[filter]->FBP.FU[1]));
				note->m_VCF[filter]->FBP.FU[0].type = ftype;
				note->m_VCF[filter]->FBP.FU[0].subtype = fsubtype;
				note->m_VCF[filter]->FBP.FU[1].type = ftype;
				note->m_VCF[filter]->FBP.FU[1].subtype = fsubtype;
				note->m_VCF[filter]->m_filterCoefficientMaker[0].Reset();
				note->m_VCF[filter]->m_filterCoefficientMaker[0].prepareForPlay(m_Set->m_nSampleRate, osFactor, m_Set->m_fMasterTune.load());
			}
			//Filter B
			if (ftype2 != fut_none) {
				memset(&note->m_VCF[filter]->FBP.FU[2], 0, sizeof(note->m_VCF[filter]->FBP.FU[2]));
				memset(&note->m_VCF[filter]->FBP.FU[3], 0, sizeof(note->m_VCF[filter]->FBP.FU[3]));
				note->m_VCF[filter]->FBP.FU[2].type = ftype2;
				note->m_VCF[filter]->FBP.FU[2].subtype = fsubtype2;
				note->m_VCF[filter]->FBP.FU[3].type = ftype2;
				note->m_VCF[filter]->FBP.FU[3].subtype = fsubtype2;
				note->m_VCF[filter]->m_filterCoefficientMaker[1].Reset();
				note->m_VCF[filter]->m_filterCoefficientMaker[1].prepareForPlay(m_Set->m_nSampleRate, osFactor, m_Set->m_fMasterTune.load());
			}

			//Filter C
			if (ftype3 != fut_none) {
				memset(&note->m_VCF[filter]->FBP.FU[4], 0, sizeof(note->m_VCF[filter]->FBP.FU[4]));
				memset(&note->m_VCF[filter]->FBP.FU[5], 0, sizeof(note->m_VCF[filter]->FBP.FU[5]));
				note->m_VCF[filter]->FBP.FU[4].type = ftype3;
				note->m_VCF[filter]->FBP.FU[4].subtype = fsubtype3;
				note->m_VCF[filter]->FBP.FU[5].type = ftype3;
				note->m_VCF[filter]->FBP.FU[5].subtype = fsubtype3;
				note->m_VCF[filter]->m_filterCoefficientMaker[2].Reset();
				note->m_VCF[filter]->m_filterCoefficientMaker[2].prepareForPlay(m_Set->m_nSampleRate, osFactor, m_Set->m_fMasterTune.load());
			}

			if (ftype == fut_vastscream)
				note->m_VCF[filter]->m_ladderFilter1.setMode(dsp::LadderFilter<float>::Mode::LPF12);
		}
	}
	else { //UI!
		//Filter A
...
int VASTQFilter::processBlock(OwnedArray<VASTSynthesiserVoice>* voices, modMatrixInputState* matrixInputState, sRoutingBuffers* routingBuffers, CVASTSettings* m_Set, int filter, dsp::AudioBlock<float> filterBlock, int startSample, int numSamples, bool isUI, bool hasNextFilter, CVASTVcf* uiVCF, bool warmup) {
	...
	while (samplesToProcess > 0) {
		...
		while (!complete)
		{
			...
			if (isUI) {
				...
				vcf = note->m_VCF[filter];
				bool bPlayiningInRange = note->isPlayingInRange(startSample, numSamples); //requires that mseg is processed before!
				doit = (bPlayiningInRange || (note->m_bLastFilterOutputZero[filter] == false));
...
int VASTQFilter::processQFilter(dsp::AudioBlock<float> filterBlock, sRoutingBuffers* routingBuffers, int numSamples, int startSample, int ftype, int fsubtype, int ftype2, int fsubtype2, int ftype3, int fsubtype3, int fws, int osFactor, OwnedArray<VASTSynthesiserVoice>* voices, int filter, float gain, float drive, float feedback, bool isUI, bool hasNextFilter, CVASTVcf* uiVCF, bool warmup) {
	...
	if (isLegacy) {
		...
		while (!complete)
		{
			...
			if (isUI) {
				...
				bool bPlayiningInRange = note->isPlayingInRange(startSample, numSamples); //requires that mseg is processed before!
				doit = (bPlayiningInRange || (note->m_bLastFilterOutputZero[filter] == false));
				if (voiceNo == note->m_Set->m_uMaxPoly - 1)
					complete = true;
			}
			if (doit) {
	...
	//per voices
		if (!isUI) {
			for (auto* voice : *voices)
			{
				CVASTSingleNote* myNote = dynamic_cast<CVASTSingleNote*>(voice);
				if (myNote == nullptr) {
					vassertfalse; //error state - thread problem
					continue;
				}
				...
				if (mVoiceNo <= highestVoicePlaying) {
					bool bPlayiningInRange = myNote->isPlayingInRange(startSample, numSamples); //requires that mseg is processed before!

					if (bPlayiningInRange || (myNote->m_bLastFilterOutputZero[filter] == false)) {
